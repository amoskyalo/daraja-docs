...
**Endpoint:** https://api.safaricom.co.ke/v1/billmanager-invoice/optin
...
| Email           | This is the official contact email address for the organization signing up to bill manager. It will appear in features sent to the customer such as invoices and payment receipts for customers to reach out to you as a business.                                                      | String\|Required  | example@email.com                     |
...
```json
{
  "app_key": "AG_237648736_126732989KJ",
  "resmsg": "Success",
  "rescode": "200"
}
```
...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ |
| app_key | This app_key is the one you receive upon onboarding your paybill to the Daraja Platform.                                                                                       | AG_237648736_126732989KJ |
...
**Endpoint:** https://api.safaricom.co.ke/v1/billmanager-invoice/single-invoicing
...
```json
{
  "externalReference": "#993340",
  "billedFullName": "John Doe",
  "billedPhoneNumber": "07XXXXXXX",
  "billedPeriod": "August 2021",
  "invoiceName": "Jentys",
  "dueDate": "2021-10-15",
  "accountReference": "1A50678H",
  "amount": "600",
  "invoiceItems": [
    {
      "itemName": "food",
...
| billedFullName    | The name of the recipient to receive the invoice details. It will appear in the sms sent.                                                                                                                            | String\|Required                              | Thomas Shelby<br/>Tasha WholeSalers                                                                           |
...
| billedPeriod      | Month and Year                                                                                                                                                                                                       | Date[M/YY]\|Required                          | August 2021                                                                                                   |
| invoiceName       | A descriptive invoice name for what your customer is being billed. it will appear in the invoice sms sent to your customer                                                                                           | Varchar\|Required                             | 1. damagefee<br/>2. watersupply                                                                               |
...
| accountReference  | The account number being invoiced that uniquely identifies a customer. It could be a customer name,business name, a property unit, a student's name etc.                                                             | String(Varchar)\|Required                     | Customer Name - John Doe<br/>Business Name - WS Suppliers<br/>Property Unit - G70<br/>Student Name - Joan Doe |
| amount            | Total invoice amount to be paid in Kenyan Shillings No special characters eg comma's should be put in the currency amount                                                                                            | Numeric\|Required                             | 1. 2000<br/>2. 500<br/>3. 200000                                                                              |
| invoiceItems      | These are additional billable items that you need included in your invoice. The invoice items will appear on the e-invoice.                                                                                          | itemName - String amount - Numeric \|Optional | Food<br/>1000                                                                                                 |
...
**Endpoint:** https://api.safaricom.co.ke/v1/billmanager-invoice/bulk-invoicing
...
**Endpoint:** https://api.safaricom.co.ke/v1/billmanager-invoice/reconciliation

The bill manager payment feature enables your customers to receive e-receipts for payments made to your paybill account.

**Pre-Condition**

Non business pay bill must have been onboarded to bill manager for us to push payments to you and for bill manager to receive your payment acknowledgement details. Please see the bill manager onboarding documentation for more details.
...
```json
{
  "transactionId": "RJB53MTR1N",
  "paidAmount": "50",
  "msisdn": "2547220000",
  "dateCreated": "2021-09-15",
  "accountReference": "IG1010789",
  "shortCode": "349350555"
}
```
</CodeExample>
...
| Name             | Description                                                                                                                                                       | Type                      | Sample Values |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------- |
| transactionId    | The M-PESA generated reference.                                                                                                                                   | String(Varchar)\|Required | RJB53MTR1N    |
...
| shortCode        | This is organizations shortcode (Paybill or Buygoods - A 5 to 6 digit account number) used to identify an organization and receive the transaction.               | Numeric                   | 450645        |
...
3. Once the payment details are availed to you, you will reconcile the payment on your end and send an acknowledgement message to the bill manager acknowledgement endpoint. Acknowledgement details sent to bill manager must contain the following:
...
4. Upon receiving the acknowledgement message, we will process it and send a receipt acknowledgement message to your customers via the phone number in the acknowledgement API result
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------- |
| Payment Date          | This is the date you expect your customer to settle the invoiced amount.<br/>Two (2) reminders shall be sent to your invoiced customer i.e. Seven (7) days prior to the due date and on the due date | Date                      | 2021-10-01                              |
...
| Transaction ID        | The M-PESA generated reference.                                                                                                                                                                      | String(Varchar)\|Required | PBS3MTR1N                               |
| Customer Phone Number | The Safaricom phone number that receives the e-invoice details via sms                                                                                                                               | Numeric\|Required         | • 0722XXXXXXX<br/>• 0710XXXXXX          |
| Customer Full Name    | The name of the invoiced recipient                                                                                                                                                                   | String(Varchar)\|Required | • Thomas Shelby<br/>• Tasha WholeSalers |
| Invoice Name          | A descriptive name for what your customer is being billed for.<br/>It will appear on the invoice sms sent to your customer.                                                                          | String(Varchar)\|Required | • damagefee<br/>• watersupply           |
...
**Endpoint:** https://api.safaricom.co.ke/v1/billmanager-invoice/cancel-single-invoice

#### Cancel Invoice - Request Body

<CodeExample title="Cancel Invoice - Request Body">
```json
{
...
The bulk cancel invoice API allows you to recall more than one sent invoice.

**Endpoint:** https://sandbox.safaricom.co.ke/v1/billmanager-invoice/cancel-bulk-invoices
...
<CodeExample title="Cancel Invoice-Error">
```json
{
...
**Endpoint:** https://sandbox.safaricom.co.ke/v1/billmanager-invoice/change-optin-details
...
| sendReminders   | This field gives you the flexibility as a business to enable or disable sms payment reminders for invoices sent. A payment reminder is sent 7 days before the due date, 3 days before the due date, and the day the payment is due.<br/>0 - Disable Reminders<br/>1- Enable Reminders    | Numeric\|Required | 0 or 1                                                         |
